...
begin
	...
	select count(empresa_id) from domicilios_pedido where empresa_id = id_emp  into t;
	SELECT COALESCE(array_to_json(array_agg(row_to_json(p))), '[]') from (
		...
		case when p.despachado then 1 else 0 end as estado,case when p.entregado then 1 else 0 end as entregado,p.id from domicilios_pedido as p 
		inner join domicilios_empleado as e1 on (p.alistador_id=e1.usuario_ptr_id and p.empresa_id=id_emp) 
		inner join domicilios_empleado as e2 on (p.motorizado_id=e2.usuario_ptr_id) inner join auth_user as u1 on (e1.usuario_ptr_id=u1.id) 
		inner join auth_user as u2 on (e2.usuario_ptr_id=u2.id)
		inner join domicilios_empresa as emp on (p.empresa_id=emp.id) inner join auth_user as a3 on (p.supervisor_id=a3.id) where 
