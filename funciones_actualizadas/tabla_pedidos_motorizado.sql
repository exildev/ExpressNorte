...
begin
	...
	SELECT COALESCE(array_to_json(array_agg(row_to_json(p))), '[]') from (
		...
		from domicilios_pedido where motorizado_id=id_des and (num_pedido like '%'||search_||'%' or npedido_express like '%'||search_||'%') order by fecha_pedido desc,entregado limit length_ offset start_
	) p into l;
