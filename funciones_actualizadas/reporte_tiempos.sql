-- Function: reporte_tiempos(integer, text, text, integer, integer)

-- DROP FUNCTION reporte_tiempos(integer, text, text, integer, integer);

CREATE OR REPLACE FUNCTION reporte_tiempos(id_des integer, search_ text, order_ text, start_ integer, length_ integer)
  RETURNS text AS
$BODY$
declare 
	l json;
...
begin
	...
	SELECT COALESCE(array_to_json(array_agg(row_to_json(p))), '[]') from (
		...
		select * from (
				select nump,cliente,supervisor,alistador,motori,total,
				...
				from (
					select p.id,p.num_pedido as nump,c.first_name||' '||c.last_name as cliente,s.first_name||' '||s.last_name as supervisor,
					       ...
					       (EXTRACT(EPOCH from t.entregado)-EXTRACT(EPOCH from t.despachado)) /60 as entrega
							...
							inner join domicilios_time as ti on(ti.pedido_id=p.id) where p.empresa_id=id_emp order by p.fecha_pedido desc) as p
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION reporte_tiempos(integer, text, text, integer, integer)
  OWNER TO postgres;
