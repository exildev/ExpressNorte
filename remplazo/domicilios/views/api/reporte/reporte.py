...
def report_empleado_pdf(request):
	...
	if len(res) > 0 :
		...
		alistar = Paragraph('''Alistamiento''',styleBH)
		despacho = Paragraph('''Despacho''',styleBH)
		entrega = Paragraph('''Entrega''',styleBH)
...
def report_empleado_excel(request):
	...
	if len(res) > 0 :
		...
		writer = csv.writer(response)
		emp = res[0]['f2']
		writer.writerow(['Cliente','Direccion','Total','Motorizado','Alistamiento','Despacho','Entrega'])
		for res in emp :
