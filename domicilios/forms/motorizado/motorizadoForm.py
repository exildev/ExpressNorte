...
class AddTecnoForm(forms.ModelForm):
    class Meta:
        ...
        "fecha_expedicionT": forms.DateInput(attrs={'placeholder': 'Fecha de Expedición', 'type': 'date'}),
        "fecha_expiracionT": forms.DateInput(attrs={'placeholder': 'Fecha de Expiracion', 'type': 'date'}),
        }
    ...
    def clean_fecha_expiracionT(self):
        ...
        if fecha_expedicionT >= fecha_expiracionT:
            raise forms.ValidationError("La fecha de Expiracion debe ser mayor que la de expedicion")
...
class AddSoatForm(forms.ModelForm):
    class Meta:
        ...
        "fecha_expedicionS": forms.DateInput(attrs={'placeholder': 'Fecha de Expedición', 'type': 'date'}),
        "fecha_expiracionS": forms.DateInput(attrs={'placeholder': 'Fecha de Expiracion', 'type': 'date'}),
        }
...
class AddMotorizadoForm(forms.ModelForm):
    empleado = forms.ModelChoiceField(queryset=Empleado.objects.filter(cargo='MOTORIZADO', motorizado__empleado__isnull=True), widget=forms.Select(attrs={'class': 'ui fluid search selection dropdown'}))
...
class editMotorizadoForm(forms.ModelForm):
    empleado = forms.ModelChoiceField(queryset=Empleado.objects.filter(cargo='MOTORIZADO'), widget=forms.Select(attrs={'class': 'ui fluid search selection dropdown'}))
    moto = forms.ModelChoiceField(queryset=Moto.objects.filter(motorizado__moto__isnull=False,estado=True), widget=forms.Select(attrs={'class': 'ui fluid search selection dropdown'}))
...
class AsignarMotoForm(forms.ModelForm):
    empleado = forms.ModelChoiceField(queryset=Empleado.objects.filter(cargo='MOTORIZADO', motorizado__empleado__isnull=True), widget=forms.Select(attrs={'class': 'ui fluid search selection dropdown'}))
    moto = forms.ModelChoiceField(queryset=Moto.objects.filter(motorizado__moto__isnull=True,estado=True), widget=forms.Select(attrs={'class': 'ui fluid search selection dropdown'}))
...
class reporteForm(forms.Form):
    ciudad = forms.ChoiceField(choices=CIUDAD, widget=forms.Select(attrs={'class': 'ui fluid search selection dropdown', 'required': ''}), required=True)
    fecha_inicio = forms.DateTimeField(widget=forms.DateTimeInput(attrs={'placeholder': 'Fecha de Inicio', 'required': '', 'type': 'date'}))
    fecha_final = forms.DateTimeField(widget=forms.DateTimeInput(attrs={'placeholder': 'Fecha Final', 'required': '', 'type': 'date'}))
