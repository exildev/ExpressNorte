function tablaPedidos(){
	window.table = $('#search-results').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "/plataforma/reporte/servi/tabla/info/empleados/",
                ...
                if (ini.val() .length > 0 && fin.val().length>0){
                    if(ini.val() < fin.val()){
                        if($(this).hasClass('excel')){
                            window.location.href = "/plataforma/reporte/report/empleado/excel/?id="+$(this).parent().find('input[type="hidden"]').val()+"&ini="+$('#inicio').val()+"&fin="+$('#inicio').val();
                        }else if($(this).hasClass('pdf')){
                            window.location.href = "/plataforma/reporte/report/empleado/pdf/?id="+$(this).parent().find('input[type="hidden"]').val()+"&ini="+$('#inicio').val()+"&fin="+$('#inicio').val();
                        }
                    }else{
